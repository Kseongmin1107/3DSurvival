...
-   **스태미나 소모**: `PlayerController.cs`의 `OnJumpInput` 함수는 `PlayerCondition.UseStamina()`를 호출하여 점프 시 스태미나를 소모하며, 스태미나가 충분할 때만 점프가 가능하도록 구현했습니다. 점프에 필요한 스태미나 양은 `jumpStaminaCost` 변수를 통해 Inspector에서 조절할 수 있습니다.
